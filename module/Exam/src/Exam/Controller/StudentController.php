<?php
	...
	class StudentController extends IndexController {
		...
		public function examAction() {
			$this->init();
			if (!$this->getAcl()->isAllowed($this->sessionStudent->right, null, "student_exam")) {
		...
		public function requireTestAgainAction() {
			$this->init();
			if (!$this->getAcl()->isAllowed($this->sessionStudent->right, null, "student_requiretestagain")) {
				$this->redirect()->toUrl('../index/login');
			}
		...
		public function createExamAction() {
			$this->init();
			if (!$this->getAcl()->isAllowed($this->sessionStudent->right, null, "student_createexam")) {
				$this->redirect()->toUrl('../index/login');
			}
			$subjectId	= $this->params('id');
			$this->testInfoTable	= $this->getServiceLocator()->get('Exam\Model\TestInfoTable');
		...
		public function finishExamAction() {
			$this->init();
			if (!$this->getAcl()->isAllowed($this->sessionStudent->right, null, "student_finishexam")) {
				$this->redirect()->toUrl('../index/login');
			}
		...
		public function showResultAction() {
			$this->init();
			if (!$this->getAcl()->isAllowed($this->sessionStudent->right, null, "student_showresult")) {
				$this->redirect()->toUrl('../index/login');
			}
		...
		public function viewAnswerAction() {
			$this->init();
			if (!$this->getAcl()->isAllowed($this->sessionStudent->right, null, "student_viewanswer")) {
				$this->redirect()->toUrl('../index/login');
			}
			$num	= $this->sessionStudent->numOfQuestion;
			$type1	= (int)($num/2);
			$type2	= (int)(($num - $type1)/2);
			$type3	= $num - $type1 - $type2;
			$form	= new ExamContentForm();
			$form->initial($type1, $type2, $type3);
